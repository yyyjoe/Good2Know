...
# minimal like of posts to be considered
minimal_like <- 100
...
extractInfo <- function(index){
    ...
    for(i in 1:maxrows){
        if(i == maxrows){
        	...
        	if(posts[i,11] < minimal_like) next
...
filename <- str_glue("data/table-{hashtag}.csv")
